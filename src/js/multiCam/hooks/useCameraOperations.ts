...
import { importAndInsertCameraVideos } from '../api';
...
export const useCameraOperations = (
  ...
  const handleDeleteCamera = async (cameraId: string) => {
    ...
    // Rename remaining cameras to maintain sequence
    const renamedCameras = updatedCameras.map((camera, index) => ({
      ...camera,
      name: generateCameraName(index)
    }));
    
    setCameras(renamedCameras);
    await saveState(renamedCameras, mainSequenceId);
  };
